...
def get_base_type(complex_type: type) -> tuple[type, tuple, bool]:
    ...
    This function handles both simple types and union types (e.g., types that
    can be None). For container types like `list[str]` or `dict[str, list[int]]`,
    it extracts the base container type.
    ...
    Args:
        complex_type (type): The complex type to be analyzed. This can be a
            simple type, a container type, or a union type with None.

    Returns:
        tuple[type, bool]: A tuple containing the base type and a boolean
            indicating if the type is optional. For example:
    ...
    Raises:
        TypeError: If the complex type is a union that does not exclusively
            involve a single type and None.
    """
    ...
    if origin is Union or origin is UnionType:
        ...
        if type(None) not in args:
            raise TypeError(
                f"Complex type must be a union with None only. This is {complex_type}"
            )
...
def get_container_base_type(type_: type) -> type:
    ...
    This helper function is used to get the base type of container types.
    For example, it extracts `list` from `list[str]` and `dict` from
    `dict[str, list[int]]`.
    ...
    Returns:
        type: The base container type if the input is a container type;
            otherwise, the input type itself. For example:
